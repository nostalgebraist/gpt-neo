...
parser.add_argument('--tpu', type=str, required=True)
# JSON file that contains model parameters
parser.add_argument('--model', type=str, required=True)
# name of experiment (will show up in omniboard)
...
parser.add_argument('--heartbeat_timeout', type=int, default=1800)
args = parser.parse_args()

params = fetch_model_params(args.model)

ex = sacred.Experiment(args.experiment_name)
ex.observers.append(sacred.observers.QueuedMongoObserver(
    url='127.0.0.1:27017', db_name='db', username='user', password='password'))
...
def train_thread(args, tpu, id, q):
    ...
    cmd = "python3 main.py --tpu {tpu} --model run_configs/config_{id}.json --steps_per_checkpoint {steps_per_checkpoint} {opts} --sacred_id {run_id}".format(
        tpu=tpu, id=id, steps_per_checkpoint=args.steps_per_checkpoint, opts=opts, run_id=id)
    print('Running:', cmd)
...
def get_scalar_data(base_uri, run, tag):
    j = get_json(f'{base_uri}/data/plugin/scalars/scalars',
                 {'experiment': '', 'run': run, 'tag': tag})
    ...
    try:
        ...
        if 'eval_lambada' in runs:
            if 'lambada_acc' in tag_sets['eval_lambada']:
                r['lambada_acc'] = get_scalar_data(
                    base_uri, 'eval_lambada', 'lambada_acc')
...
def main(_run):
    ...
    shutil.copy(args.model if args.model.endswith(
        '.json') else 'configs/{}.json'.format(args.model), 'run_configs/config_{}.json'.format(_run._id))
    ...
    os.system("screen -S tensorboard_{} -d -m bash -c 'tensorboard --logdir {} --port {} --bind_all --reload_multifile=true || tensorboard --logdir {} --port {} --reload_multifile=true'".format(
        _run._id, params["model_path"], tensorboard_port, params["model_path"], tensorboard_port,))
    ...
    while True:
        ...
        q = queue.Queue()
        trainthd = threading.Thread(
            target=train_thread, args=(args, args.tpu, _run._id, q))
        trainthd.start()
        ...
        while trainthd.is_alive():
            ...
            for k in data.keys():
                for ts, step, val in data[k]:
                    ...
                    if k == 'loss':
                        ...
                        print('Logged to sacred: step={},loss={},tb_ts={}'.format(
                            step, val, ts))
            ...
            if os.path.exists(f'eval_{_run._id}.jsonl'):
                with open(f'eval_{_run._id}.jsonl') as fh:
                    for line in fh:
                        ...
                        for metr in ob.keys():
                            k = 'fs.' + val_task + '.' + metr
                            if metr in ['task', 'global_step']:
                                continue
                            if val_step <= curr_step.get(k, -1):
                                continue
            ...
            if time.time() - last_tb_log_time > heartbeat_timeout:
                ...
                while trainthd.is_alive():
                    print(
                        'logging thread waiting for killing stalled run and for tpu recreate to finish')
                    time.sleep(60)
